...
class UserProfileInfoCubit extends Cubit<UserProfileInfoState> {
  UserProfileInfoCubit({UserProfileInfoState initialState = const UserProfileInfoInitialState()}) : super(initialState) {
    loadUserProfileInfo();
  }
  ...
  Future<void> loadUserProfileInfo() async {
    ...
    try {
      ...
      emit(UserProfileInfoLoadedState(user.givenName, user.familyName, user.name, user.nickname, user.email, user.picture));
    } catch (e) {
